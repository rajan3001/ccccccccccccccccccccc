...
export default function CurrentAffairsPage() {
  ...
  useEffect(() => {
    if (!selectedDate && !latestLoading) {
      setSelectedDate(todayStr);
    }
  }, [latestLoading, selectedDate, todayStr]);
  ...
  useEffect(() => {
    if (
      ...
      !autoGeneratedRef.current.has(dateStr)
    ) {
      autoGeneratedRef.current.add(dateStr);
      generateMutation.mutate(
        { date: dateStr, stateFilter: null },
        {
          onError: (error: Error) => {
            toast({
  ...
  }, [dateReady, isLoading, hasDigest, dateStr]);
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  {generateMutation.isPending ? (
                    <>
                      ...
                      <h3 className="text-lg font-semibold mb-2" data-testid="text-generating-digest">
                        Generating digest for {formatDisplayDate(dateStr)}
                      </h3>
                      <p className="text-muted-foreground text-sm max-w-md">
                        Our AI is curating the latest news. This may take a moment...
                      </p>
                    </>
                  ) : isFuture(dateStr) ? (
                    <>
                      ...
                      <h3 className="text-lg font-semibold mb-2" data-testid="text-no-digest">
                        Today's current affairs will be available soon
                      </h3>
                      <p className="text-muted-foreground text-sm mb-5 max-w-md">
                        Check back later once our AI curates the latest news from leading newspapers.
                      </p>
                      {latestDate?.date && (
                        <Button
                          variant="outline"
                    ...
                    <>
                      ...
                      <h3 className="text-lg font-semibold mb-2" data-testid="text-no-digest">
                        Could not generate digest for {formatDisplayDate(dateStr)}
                      </h3>
                      <p className="text-muted-foreground text-sm mb-5 max-w-md">
                        The content for this date could not be fetched. Try again or pick a different date.
                      </p>
                      <Button
                        onClick={() => {
                          autoGeneratedRef.current.delete(dateStr);
                          handleGenerate();
                        }}
                        variant="outline"
                        className="gap-2"
