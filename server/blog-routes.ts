...
function renderBlogListHtml(posts: any[], page: number, totalPages: number, activeCategory: string, categoryCounts: Record<string, number>, isLoggedIn: boolean = false): string {
  ...
  const postCards = posts.length > 0 ? posts.map(post => {
    ...
    return `
    <article class="blog-card" data-category="${post.category}" data-date="${new Date(post.publishedAt).toISOString()}" data-tags="${(post.tags || []).join(',')}" itemscope itemtype="https://schema.org/Article">
...
<head>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .top-bar{background:hsla(40,33%,98%,0.92);backdrop-filter:blur(20px) saturate(1.2);-webkit-backdrop-filter:blur(20px) saturate(1.2);border-bottom:1px solid var(--border);padding:0 max(1rem,calc((100% - 1200px)/2 + 1.5rem));display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:50;height:var(--header-h);gap:0.5rem}
    ...
    .top-bar-logo .ai-text{color:var(--primary)}
    ...
    .top-bar-nav a:hover{color:var(--primary);background:var(--primary-dim)}
    .top-bar-nav a.nav-active{color:var(--primary-dark);font-weight:600}
    .nav-cta{display:inline-flex;align-items:center;gap:0.35rem;background:var(--primary);color:#fff!important;font-size:0.82rem;font-weight:600;padding:0.45rem 0.75rem;border-radius:0.5rem;text-decoration:none;transition:background 0.2s;margin-left:0.25rem;border:none;cursor:pointer;white-space:nowrap;flex-shrink:0}
    ...
    .cat-scroll-area{overflow-x:auto;scrollbar-width:none;flex:1;min-width:0}
    ...
    .cat-arrow{width:36px;height:36px;border-radius:50%;border:1px solid var(--border);background:var(--bg-card);display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;transition:all 0.2s;color:var(--text-secondary)}
    .cat-arrow:hover{border-color:var(--primary-border);color:var(--primary);background:var(--primary-dim)}
    .cat-arrow.hidden{opacity:0;pointer-events:none}
    .cat-arrow svg{width:16px;height:16px}
    .cat-tab{display:inline-flex;align-items:center;gap:0.45rem;padding:0.5rem 1.05rem;border-radius:9999px;font-size:0.8rem;font-weight:600;color:var(--text-secondary);background:hsla(25,20%,50%,0.06);border:1px solid hsla(25,20%,50%,0.1);transition:all 0.22s cubic-bezier(0.34,1.56,0.64,1);white-space:nowrap;flex-shrink:0;cursor:pointer;font-family:inherit;position:relative;overflow:hidden;letter-spacing:0.01em}
    ...
    .cat-tab:hover{color:var(--text);background:hsla(25,20%,50%,0.1);border-color:hsla(25,20%,50%,0.18);transform:translateY(-1px);box-shadow:0 3px 10px rgba(0,0,0,0.08)}
    .cat-tab:hover::after{opacity:1}
    .cat-tab:active{animation:tab-pop 0.2s ease-out both}
    .cat-active{background:linear-gradient(135deg,hsl(25,85%,42%),hsl(30,90%,48%))!important;color:#fff!important;border-color:transparent!important;font-weight:700;box-shadow:0 4px 14px hsla(25,85%,45%,0.35),0 1px 3px hsla(25,85%,45%,0.2),inset 0 1px 0 rgba(255,255,255,0.15)!important;transform:translateY(-1px)}
    ...
    .main{max-width:1200px;margin:0 auto;padding:2rem 1.5rem 3rem;display:flex;gap:2rem}
    ...
    .blog-card:hover{border-color:var(--border-hover);transform:translateY(-5px) scale(1)!important;box-shadow:0 16px 48px hsla(25,30%,30%,0.1),0 0 0 1px hsla(25,85%,45%,0.06)}
    ...
    .blog-card-placeholder{display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:linear-gradient(135deg,hsl(30,25%,92%),hsl(25,20%,88%))}
    .placeholder-label{font-size:0.82rem;font-weight:600;color:var(--text-muted);letter-spacing:0.04em;opacity:0.5}
    .blog-card-badge{position:absolute;top:0.75rem;left:0.75rem;background:hsla(40,33%,98%,0.85);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);color:var(--primary-dark);font-size:0.68rem;font-weight:600;padding:0.25rem 0.65rem;border-radius:6px;letter-spacing:0.03em;border:1px solid var(--primary-border)}
    ...
    .blog-back-link{display:inline-block;color:var(--primary-dark);text-decoration:none;font-weight:500;border:1px solid var(--primary-border);padding:0.5rem 1.25rem;border-radius:0.5rem;transition:all 0.25s}
    .blog-back-link:hover{background:var(--primary-dim)}
    ...
    .pagination a.active{background:var(--primary);color:#fff;font-weight:700;border-color:var(--primary)}
    ...
    .sidebar-card{background:#fff;border:1px solid var(--border);border-radius:10px;margin-bottom:1rem;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.04);padding:1rem}
    ...
    .sb-archive-header.blue{background:linear-gradient(135deg,hsl(215,90%,30%),hsl(215,85%,42%))}
    ...
    .sb-archive-header span{position:relative;z-index:1;font-size:0.78rem;letter-spacing:0.02em;text-transform:uppercase}
    .sb-archive-header svg{width:16px;height:16px;position:relative;z-index:1;stroke-width:2.5}
    ...
    .sb-go-btn{width:100%;padding:0.55rem;background:hsl(215,90%,45%);color:#fff;border:none;border-radius:6px;font-weight:700;font-size:0.8rem;cursor:pointer;transition:all 0.2s;font-family:var(--font-display);letter-spacing:0.02em;text-transform:uppercase}
    .sb-go-btn:hover{background:hsl(215,90%,35%);transform:translateY(-1px);box-shadow:0 4px 12px hsla(215,90%,45%,0.35)}
    ...
    .site-footer::before{content:'';position:absolute;top:0;left:50%;transform:translateX(-50%);width:120px;height:1px;background:linear-gradient(90deg,transparent,var(--primary),transparent)}
    ...
    .footer-brand .ai-text{color:var(--primary-dark)}
...
<body>
  ...
  <div class="cat-strip-wrap">
    <div class="cat-strip" id="catStripScroll">
      <button class="cat-arrow" id="catArrowL" data-testid="cat-arrow-left" aria-label="Scroll tabs left">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
      </button>
      <div class="cat-scroll-area" id="catScrollArea">
        <div class="cat-strip-inner" id="catTabs">${categoryTabs}</div>
      </div>
      <button class="cat-arrow" id="catArrowR" data-testid="cat-arrow-right" aria-label="Scroll tabs right">
  ...
  <main class="main">
    ...
    <aside class="sidebar" data-testid="blog-sidebar">
      ...
      ${topTags.length > 0 ? `<div class="sidebar-card" data-testid="sidebar-popular-tags">
        <h3>Popular Tags</h3>
        <div class="tag-pills">
          ${topTags.map(([tag, count]) => `<a class="tag-pill" href="/blog?tag=${encodeURIComponent(tag)}" data-testid="tag-pill-${tag}">${tag}</a>`).join('')}
        </div>
      </div>` : ''}

      <div class="sidebar-card cta-card" data-testid="sidebar-cta">
        <h3>Start Your UPSC Journey</h3>
        <p>Get personalized AI-powered study plans, practice quizzes, and expert-curated content.</p>
        <a href="/login" class="cta-btn" data-testid="sidebar-cta-login">
          Get Started
  ...
  (function(){
    ...
    var catScrollArea=document.getElementById('catScrollArea');
    var arrowL=document.getElementById('catArrowL');
    var arrowR=document.getElementById('catArrowR');
    function updateArrows(){
      if(!catScrollArea)return;
      var sl=catScrollArea.scrollLeft;
      var maxSl=catScrollArea.scrollWidth-catScrollArea.clientWidth;
      if(arrowL)arrowL.classList.toggle('hidden',sl<=8);
      if(arrowR)arrowR.classList.toggle('hidden',sl>=maxSl-8);
    }
    if(catScrollArea){
      catScrollArea.addEventListener('scroll',updateArrows);
      updateArrows();
      window.addEventListener('resize',updateArrows);
    }
    if(arrowL)arrowL.addEventListener('click',function(){catScrollArea.scrollBy({left:-200,behavior:'smooth'})});
    if(arrowR)arrowR.addEventListener('click',function(){catScrollArea.scrollBy({left:200,behavior:'smooth'})});

    var params=new URLSearchParams(window.location.search);
    var initCat=params.get('category');
    var initDate=params.get('date');
    var initTag=params.get('tag');
    var activeDate=null;

    function filterCards(cat, dateFilter, tagFilter){
      current=cat||'all';
      ...
      setTimeout(function(){
        ...
        cards.forEach(function(c){
          var catOk=current==='all'||c.getAttribute('data-category')===current;
          var dateOk=true;
          if(dateFilter){
            var pubDate=c.getAttribute('data-date');
            dateOk=pubDate&&pubDate.substring(0,10)===dateFilter;
          }
          var tagOk=true;
          if(tagFilter){
            var cardTags=(c.getAttribute('data-tags')||'').toLowerCase();
            tagOk=cardTags.indexOf(tagFilter.toLowerCase())!==-1;
          }
          var show=catOk&&dateOk&&tagOk;
          if(show){
            c.style.display='';
            visible++;
            var d=delay;
        ...
        setTimeout(function(){
          var lbl=catLabels[current]||'All Articles';
          if(dateFilter)lbl=dateFilter;
          if(tagFilter)lbl='#'+tagFilter;
          label.textContent=lbl;
          count.textContent=visible+' article'+(visible!==1?'s':'');
          if(heroP)heroP.textContent=current!=='all'?(catDescs[current]||''):'In-depth articles on strategy, current affairs, subject guides, and answer writing to accelerate your preparation.';
      ...
      if(tabs)tabs.querySelectorAll('.cat-tab').forEach(function(t){
        t.classList.toggle('cat-active',t.getAttribute('data-cat')===current);
      });

      document.querySelectorAll('.date-item').forEach(function(d){
        d.classList.toggle('active',d.getAttribute('href')==='/blog?date='+dateFilter);
      });
    }
    ...
    if(tabs)tabs.addEventListener('click',function(e){
      ...
      if(!btn)return;
      e.preventDefault();
      filterCards(btn.getAttribute('data-cat'),null,null);
    });
    ...
    document.querySelectorAll('.date-item').forEach(function(d){
      d.addEventListener('click',function(e){
        ...
        var dv=href?href.split('date=')[1]:null;
        if(dv)filterCards('all',dv,null);
      });
    });

    document.querySelectorAll('.tag-pill').forEach(function(t){
      t.addEventListener('click',function(e){
        e.preventDefault();
        filterCards('all',null,t.textContent.trim());
      });
    });

    if(initDate){
      filterCards('all',initDate,null);
    } else if(initTag){
      filterCards('all',null,initTag);
    } else if(initCat && catLabels[initCat]){
      filterCards(initCat,null,null);
    }
  })();
  </script>
</body>
...
<head>
  ...
  <meta property="og:site_name" content="Learnpro AI">
  ${post.coverImageUrl ? `<meta property="og:image" content="${SITE_URL}${post.coverImageUrl}">
  <meta property="og:image:width" content="1200">
  <meta property="og:image:height" content="630">` : ''}
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="${escapedTitle}">
  <meta name="twitter:description" content="${escapedDesc}">
  ${post.coverImageUrl ? `<meta name="twitter:image" content="${SITE_URL}${post.coverImageUrl}">` : ''}
  ...
  <script type="application/ld+json">
  {
    ...
    ${post.coverImageUrl ? `"image": {"@type":"ImageObject","url":"${SITE_URL}${post.coverImageUrl}","width":1200,"height":630},` : ''}
    "author": {"@type": "Organization","name": "Learnpro AI","url": "${SITE_URL}"},
    "publisher": {"@type": "Organization","name": "Learnpro AI","url": "${SITE_URL}","logo":{"@type":"ImageObject","url":"${SITE_URL}/favicon_final.webp","width":64,"height":64}},
  ...
  <script type="application/ld+json">
  {
    ...
    "itemListElement": [
      ...
      {"@type":"ListItem","position":3,"name":"${catInfo.label}","item":"${SITE_URL}/blog?category=${post.category}"},
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .top-bar-logo img{width:28px;height:28px;object-fit:contain}
    .top-bar-logo .ai-text{color:var(--accent)}
    .top-bar-right{display:flex;align-items:center;gap:0.25rem;flex-shrink:0}
    .top-bar-nav{display:flex;gap:0.15rem;align-items:center}
    ...
    .top-bar-nav a:hover{color:var(--accent);background:var(--accent-bg)}
    .nav-cta{display:inline-flex;align-items:center;gap:0.35rem;background:var(--accent);color:#fff!important;font-size:0.82rem;font-weight:600;padding:0.45rem 0.75rem;border-radius:var(--radius);text-decoration:none;transition:background 0.2s;margin-left:0.25rem;white-space:nowrap;flex-shrink:0}
    .nav-cta:hover{background:var(--accent-dark)}
    .nav-cta svg{width:15px;height:15px;flex-shrink:0}
    .read-bar{position:fixed;top:var(--header-h);left:0;height:3px;background:linear-gradient(90deg,var(--accent-dark),var(--accent),var(--accent-light));z-index:99;transition:width 0.1s linear;width:0;border-radius:0 2px 2px 0}
    ...
    .cat-badge{display:inline-flex;align-items:center;gap:0.3rem;font-size:0.72rem;font-weight:600;padding:0.25rem 0.7rem;border-radius:var(--radius);text-decoration:none;text-transform:uppercase;letter-spacing:0.04em;color:#fff;background:var(--accent);transition:background 0.2s}
    .cat-badge:hover{background:var(--accent-dark)}
    .gs-tag{display:inline-flex;align-items:center;gap:0.25rem;font-size:0.7rem;font-weight:600;padding:0.2rem 0.6rem;border-radius:var(--radius);border:1px solid var(--accent-border);color:var(--accent);background:var(--accent-bg);text-decoration:none;transition:all 0.2s}
    .gs-tag:hover{background:hsla(25,85%,45%,0.12)}
    ...
    .article-body{padding:1.5rem 0;font-family:var(--font-body);font-size:0.95rem;line-height:1.85;color:#374151}
    ...
    .article-body a{color:var(--accent);text-decoration:underline;text-underline-offset:3px;text-decoration-color:hsla(25,85%,45%,0.3);transition:text-decoration-color 0.2s}
    .article-body a:hover{text-decoration-color:var(--accent)}
    .article-body blockquote{border-left:3px solid var(--accent);padding:0.85rem 1.25rem;margin:1.5rem 0;background:var(--accent-bg);border-radius:0 var(--radius) var(--radius) 0;font-style:italic;color:var(--text-secondary)}
    .article-body table{width:100%;border-collapse:separate;border-spacing:0;margin:1.25rem 0;font-family:var(--font-body);font-size:0.88rem;border:1px solid rgba(0,0,0,0.12);border-radius:8px;overflow:hidden}
    .article-body th{background:hsl(25,85%,42%);color:#fff;padding:0.6rem 0.85rem;text-align:left;font-weight:600;font-size:0.82rem;border:1px solid rgba(0,0,0,0.12)}
    .article-body td{padding:0.55rem 0.85rem;border:1px solid rgba(0,0,0,0.12)}
    .article-body tr:nth-child(even) td{background:hsla(25,85%,45%,0.04)}
    .article-body code{background:hsla(25,85%,45%,0.06);padding:0.15rem 0.4rem;border-radius:4px;font-size:0.88em}
    ...
    .section-head{font-family:var(--font-display);font-size:1.55rem;font-weight:800;color:var(--accent-dark);line-height:1.3;margin-bottom:0.85rem;padding-bottom:0.55rem;border-bottom:3px solid var(--accent)}
    ...
    .article-body .section-body h3{font-family:var(--font-display);font-size:1.18rem;color:var(--text);margin:1.5rem 0 0.6rem;font-weight:700;padding-left:0.75rem;border-left:3px solid var(--accent-light)}
    ...
    .info-box-blue{background:hsla(25,85%,45%,0.05);border:1px solid hsla(25,85%,45%,0.12)}
    ...
    .faq-title svg{width:20px;height:20px;color:var(--accent)}
    ...
    .faq-item.faq-open{border-color:hsla(25,85%,45%,0.25)}
    ...
    .faq-q:hover{background:hsla(25,85%,45%,0.03)}
    .faq-q svg{width:18px;height:18px;flex-shrink:0;color:var(--accent);transition:transform 0.3s cubic-bezier(0.4,0,0.2,1)}
    ...
    /* Source attribution */
    .source-box{display:flex;align-items:center;gap:0.5rem;padding:0.65rem 1rem;background:hsla(25,85%,45%,0.04);border:1px solid hsla(25,85%,45%,0.1);border-radius:var(--radius);margin:1.25rem 0;font-family:var(--font-body);font-size:0.82rem;color:var(--text-secondary)}
    .source-box svg{width:14px;height:14px;color:var(--accent);flex-shrink:0}
    ...
    .tags-list a{display:inline-block;background:var(--accent-bg);color:var(--accent);padding:0.25rem 0.65rem;border-radius:var(--radius);font-size:0.78rem;border:1px solid var(--accent-border);text-decoration:none;transition:all 0.2s;font-weight:500}
    .tags-list a:hover{background:hsla(25,85%,45%,0.12);border-color:hsla(25,85%,45%,0.3)}
    ...
    .nav-post:hover{border-color:var(--accent-border);background:var(--accent-bg)}
    .nav-post-label{font-size:0.7rem;font-weight:600;text-transform:uppercase;letter-spacing:0.06em;color:var(--accent);margin-bottom:0.3rem;display:flex;align-items:center;gap:0.3rem}
    ...
    .cta-box{margin:1.5rem 0;padding:1.75rem;border-radius:var(--radius);background:linear-gradient(135deg,var(--accent-dark),var(--accent),var(--accent-light));color:#fff;text-align:center;position:relative;overflow:hidden}
    ...
    .cta-btn{display:inline-block;background:#fff;color:var(--accent-dark);padding:0.6rem 1.5rem;border-radius:var(--radius);text-decoration:none;font-weight:700;font-size:0.88rem;transition:all 0.2s;position:relative;box-shadow:0 2px 12px rgba(0,0,0,0.1)}
    ...
    .sb-card-head{padding:0.7rem 1rem;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:0.5rem;background:linear-gradient(135deg,var(--accent-bg),hsla(25,85%,45%,0.02))}
    .sb-card-head svg{width:16px;height:16px;color:var(--accent);flex-shrink:0}
    ...
    .toc-item a:hover,.toc-item a.toc-active{color:var(--accent);background:var(--accent-bg);border-left-color:var(--accent)}
    ...
    .sb-related-item:hover{background:var(--accent-bg)}
    ...
    .sb-related-cat{color:var(--accent);font-weight:600}
    ...
    .sb-tag{display:inline-block;padding:0.2rem 0.55rem;font-size:0.72rem;border-radius:var(--radius);background:var(--accent-bg);color:var(--accent);border:1px solid var(--accent-border);text-decoration:none;font-weight:500;transition:all 0.2s}
    .sb-tag:hover{background:hsla(25,85%,45%,0.12)}
    ...
    .sb-cta a{display:block;background:var(--accent);color:#fff;padding:0.55rem 1rem;border-radius:var(--radius);text-decoration:none;font-weight:600;font-size:0.82rem;transition:background 0.2s}
    .sb-cta a:hover{background:var(--accent-dark)}
    ...
    .footer-brand{display:flex;align-items:center;gap:0.5rem;color:var(--text);font-weight:700;font-size:0.95rem;text-decoration:none}
    .footer-brand img{width:24px;height:24px;object-fit:contain}
    .footer-brand .ai-text{color:var(--accent)}
    ...
    .share-btn.email:hover{color:#ea4335;border-color:#ea4335;background:rgba(234,67,53,0.06)}
    .share-btn.copy-link:hover{color:var(--accent);border-color:var(--accent)}
    .copy-tooltip{position:absolute;bottom:calc(100% + 6px);left:50%;transform:translateX(-50%) scale(0.9);background:var(--text);color:#fff;font-size:0.68rem;font-weight:600;padding:0.25rem 0.55rem;border-radius:4px;white-space:nowrap;opacity:0;pointer-events:none;transition:all 0.2s}
    .copy-tooltip.show{opacity:1;transform:translateX(-50%) scale(1)}
    .pdf-btn{display:inline-flex;align-items:center;gap:0.35rem;padding:0.4rem 0.85rem;border-radius:9999px;border:1px solid var(--border);background:#fff;color:var(--text-secondary);font-size:0.76rem;font-weight:600;cursor:pointer;transition:all 0.2s;text-decoration:none;font-family:var(--font-body);white-space:nowrap}
    .pdf-btn:hover{border-color:var(--accent);color:var(--accent);background:var(--accent-bg);transform:translateY(-2px);box-shadow:0 4px 12px hsla(25,85%,45%,0.15)}
    ...
    .ask-ai-dialog-overlay{display:none;position:fixed;inset:0;background:rgba(10,15,30,0.55);z-index:9999;align-items:center;justify-content:center;backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px)}
    .ask-ai-dialog-overlay.open{display:flex;animation:overlayIn 0.3s ease}
    @keyframes overlayIn{from{opacity:0}to{opacity:1}}
    .ask-ai-dialog{background:linear-gradient(170deg,#ffffff 0%,#f8faff 100%);border-radius:16px;width:92%;max-width:500px;box-shadow:0 24px 80px rgba(0,0,0,0.18),0 0 0 1px rgba(30,64,175,0.06);padding:0;position:relative;animation:dialogIn 0.35s cubic-bezier(0.34,1.56,0.64,1);overflow:hidden}
    @keyframes dialogIn{from{opacity:0;transform:scale(0.88) translateY(20px)}to{opacity:1;transform:scale(1) translateY(0)}}
    .ask-ai-dialog-header{background:linear-gradient(135deg,hsl(215,90%,30%) 0%,hsl(225,85%,42%) 50%,hsl(215,80%,50%) 100%);padding:1.15rem 1.35rem;position:relative;overflow:hidden}
    .ask-ai-dialog-header::before{content:'';position:absolute;top:-40%;right:-20%;width:180px;height:180px;border-radius:50%;background:radial-gradient(circle,rgba(255,255,255,0.08) 0%,transparent 70%);pointer-events:none}
    .ask-ai-dialog-header::after{content:'';position:absolute;bottom:-30%;left:10%;width:120px;height:120px;border-radius:50%;background:radial-gradient(circle,rgba(255,255,255,0.05) 0%,transparent 70%);pointer-events:none}
    .ask-ai-dialog-title{font-family:var(--font-display);font-size:1.05rem;font-weight:700;color:#fff;margin:0;display:flex;align-items:center;gap:0.5rem;position:relative;z-index:1}
    .ask-ai-dialog-title svg{width:22px;height:22px;color:#93c5fd;flex-shrink:0;animation:sparkleRotate 4s ease-in-out infinite;filter:drop-shadow(0 0 4px rgba(147,197,253,0.5))}
    @keyframes sparkleRotate{0%,100%{transform:rotate(0deg) scale(1)}25%{transform:rotate(15deg) scale(1.1)}50%{transform:rotate(0deg) scale(1)}75%{transform:rotate(-10deg) scale(1.05)}}
    .ask-ai-dialog-sub{font-size:0.76rem;color:rgba(191,219,254,0.85);margin:0.3rem 0 0;position:relative;z-index:1;font-weight:400;line-height:1.4}
    .ask-ai-dialog-body{padding:1.25rem 1.35rem 1.15rem}
    .ask-ai-dialog-input{width:100%;padding:0.75rem 0.9rem;border:2px solid hsl(215,30%,88%);border-radius:10px;font-size:0.88rem;font-family:var(--font-body);color:hsl(215,25%,20%);outline:none;transition:all 0.25s cubic-bezier(0.4,0,0.2,1);box-sizing:border-box;resize:none;min-height:76px;background:#fff;line-height:1.55}
    .ask-ai-dialog-input:focus{border-color:hsl(215,90%,52%);box-shadow:0 0 0 4px hsla(215,90%,52%,0.12),0 2px 8px hsla(215,90%,45%,0.08);background:#fff}
    .ask-ai-dialog-input::placeholder{color:hsl(215,15%,62%);font-style:italic;font-size:0.83rem}
    .ask-ai-dialog-actions{display:flex;align-items:center;justify-content:flex-end;gap:0.65rem;margin-top:1rem;padding-top:0.85rem;border-top:1px solid hsl(215,30%,93%)}
    .ask-ai-dialog-cancel{padding:0.5rem 1.1rem;border-radius:9px;border:1.5px solid hsl(215,20%,85%);background:#fff;color:hsl(215,15%,42%);font-size:0.82rem;font-weight:500;cursor:pointer;transition:all 0.2s;font-family:var(--font-body)}
    .ask-ai-dialog-cancel:hover{background:hsl(215,30%,96%);border-color:hsl(215,20%,75%);transform:translateY(-1px)}
    .ask-ai-dialog-cancel:active{transform:translateY(0)}
    .ask-ai-dialog-go{padding:0.5rem 1.35rem;border-radius:9px;border:none;background:linear-gradient(135deg,hsl(215,90%,48%),hsl(225,85%,55%));color:#fff;font-size:0.84rem;font-weight:600;cursor:pointer;transition:all 0.25s cubic-bezier(0.4,0,0.2,1);font-family:var(--font-body);display:inline-flex;align-items:center;gap:0.4rem;box-shadow:0 2px 8px hsla(215,90%,45%,0.25);position:relative;overflow:hidden}
    .ask-ai-dialog-go::before{content:'';position:absolute;top:0;left:-100%;width:100%;height:100%;background:linear-gradient(90deg,transparent,rgba(255,255,255,0.15),transparent);transition:left 0.5s}
    .ask-ai-dialog-go:hover{transform:translateY(-1px);box-shadow:0 4px 16px hsla(215,90%,45%,0.35);background:linear-gradient(135deg,hsl(215,90%,52%),hsl(225,85%,58%))}
    .ask-ai-dialog-go:hover::before{left:100%}
    .ask-ai-dialog-go:active{transform:translateY(0);box-shadow:0 1px 4px hsla(215,90%,45%,0.2)}
    .ask-ai-dialog-go:disabled{opacity:0.5;cursor:not-allowed;transform:none;box-shadow:none}
    .ask-ai-dialog-close{position:absolute;top:0.85rem;right:0.85rem;width:30px;height:30px;border-radius:50%;border:none;background:rgba(255,255,255,0.12);color:rgba(255,255,255,0.7);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.2s;z-index:2}
    .ask-ai-dialog-close:hover{background:rgba(255,255,255,0.22);color:#fff;transform:rotate(90deg)}
    .ask-ai-dialog-close svg{width:15px;height:15px}
    .ask-ai-dialog-login{text-align:center;padding:0.75rem 0 0.25rem}
    .ask-ai-dialog-login p{font-size:0.85rem;color:hsl(215,15%,42%);margin:0 0 0.65rem}
    .ask-ai-field-label{display:block;font-size:0.8rem;font-weight:600;color:hsl(215,15%,38%);margin-bottom:0.5rem;letter-spacing:0.01em}
    .ask-ai-phone-row{display:flex;align-items:center;gap:0;border:2px solid hsl(215,30%,88%);border-radius:10px;overflow:hidden;transition:all 0.25s}
    .ask-ai-phone-row:focus-within{border-color:hsl(215,90%,52%);box-shadow:0 0 0 4px hsla(215,90%,52%,0.12)}
    .ask-ai-phone-prefix{padding:0.65rem 0.7rem;background:hsl(215,30%,96%);font-size:0.85rem;font-weight:700;color:hsl(215,25%,35%);border-right:1.5px solid hsl(215,30%,88%);flex-shrink:0}
    .ask-ai-phone-row .ask-ai-dialog-input{border:none;border-radius:0;min-height:auto;box-shadow:none;font-weight:500}
    .ask-ai-phone-row .ask-ai-dialog-input:focus{box-shadow:none}
    .ask-ai-error{font-size:0.78rem;color:#dc2626;margin-top:0.45rem;padding:0.35rem 0.6rem;background:#fef2f2;border-radius:8px;border-left:3px solid #dc2626;animation:shakeIn 0.4s ease}
    @keyframes shakeIn{0%{transform:translateX(-8px);opacity:0}25%{transform:translateX(4px)}50%{transform:translateX(-2px)}75%{transform:translateX(1px)}100%{transform:translateX(0);opacity:1}}
    .ask-ai-otp-row{display:flex;gap:0.5rem;justify-content:center;animation:fadeUp 0.35s ease}
    @keyframes fadeUp{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
    .ask-ai-otp-input{width:42px;height:48px;text-align:center;font-size:1.15rem;font-weight:700;border:2px solid hsl(215,30%,88%);border-radius:10px;outline:none;transition:all 0.25s;font-family:var(--font-body);color:hsl(215,25%,20%);background:#fff}
    .ask-ai-otp-input:focus{border-color:hsl(215,90%,52%);box-shadow:0 0 0 4px hsla(215,90%,52%,0.12);transform:translateY(-2px)}
    .ask-ai-response-q{font-size:0.82rem;font-weight:600;color:hsl(215,80%,40%);margin-bottom:0.75rem;padding:0.55rem 0.75rem;background:linear-gradient(135deg,hsla(215,90%,52%,0.06),hsla(225,85%,55%,0.04));border-radius:10px;border-left:3px solid hsl(215,90%,52%);animation:fadeUp 0.3s ease}
    .ask-ai-response-body{font-size:0.86rem;line-height:1.75;color:hsl(215,20%,22%);max-height:380px;overflow-y:auto;padding:0.25rem 0.15rem;scrollbar-width:thin;scrollbar-color:hsl(215,30%,85%) transparent}
    .ask-ai-response-body::-webkit-scrollbar{width:5px}
    .ask-ai-response-body::-webkit-scrollbar-thumb{background:hsl(215,30%,82%);border-radius:10px}
    .ask-ai-response-body p{margin:0.55rem 0;color:hsl(215,20%,22%)}
    .ask-ai-response-body strong{color:hsl(215,60%,30%);font-weight:700}
    .ask-ai-response-body h3,.ask-ai-response-body h4{color:hsl(215,70%,32%)}
    .ask-ai-response-body ul,.ask-ai-response-body ol{padding-left:1.2rem;margin:0.55rem 0}
    .ask-ai-response-body li{margin:0.3rem 0;color:hsl(215,20%,22%)}
    .ask-ai-response-body li::marker{color:hsl(215,90%,52%)}
    .ask-ai-cursor{color:hsl(215,90%,52%);animation:cursorBlink 0.8s ease-in-out infinite;font-weight:300}
    @keyframes cursorBlink{0%,100%{opacity:1}50%{opacity:0}}
    .ask-ai-loading{display:flex;flex-direction:column;align-items:center;gap:0.75rem;padding:2rem 0;color:hsl(215,15%,50%);font-size:0.82rem;animation:fadeUp 0.3s ease}
    .ask-ai-loading-dots{display:flex;gap:6px}
    .ask-ai-loading-dots span{width:8px;height:8px;border-radius:50%;background:linear-gradient(135deg,hsl(215,90%,52%),hsl(225,85%,55%));animation:dotBounce 1.4s ease-in-out infinite}
    .ask-ai-loading-dots span:nth-child(2){animation-delay:0.16s}
    .ask-ai-loading-dots span:nth-child(3){animation-delay:0.32s}
    @keyframes dotBounce{0%,80%,100%{transform:translateY(0) scale(0.8);opacity:0.4}40%{transform:translateY(-10px) scale(1.1);opacity:1}}
    @media(prefers-reduced-motion:reduce){.ask-ai-dialog,.ask-ai-dialog-overlay.open,.ask-ai-dialog-title svg,.ask-ai-loading-dots span,.ask-ai-cursor,.ask-ai-otp-row,.ask-ai-error,.ask-ai-dialog-go::before{animation:none!important;transition:none!important}}
    ...
    @media print{
      ...
      .article-header h1{font-size:18pt;color:#000}
      .article-body{font-size:10.5pt}
      .article-body table{page-break-inside:avoid}
      .article-body th{background:#333!important;color:#fff!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}
      .article-body tr:nth-child(even) td{background:#f5f5f5!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}
      .section-head{font-size:14pt;border-bottom-color:#333}
      .cover-img{max-height:250px;overflow:hidden}
      a{color:#000;text-decoration:none}
      .mains-pq-box{border:1px solid #ccc;page-break-inside:avoid}
      .mains-pq-header{background:#444!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}
    }
  </style>
</head>
<body>
  ...
  <div class="page-wrapper">
    <main class="main-col">
      ...
      <header class="article-header" data-testid="article-header">
        ...
        <div class="share-bar" data-testid="share-bar-top">
          <span class="share-label">Share</span>
          <a href="https://api.whatsapp.com/send?text=${encodeURIComponent(post.title + ' — Read on Learnpro AI: ${SITE_URL}/blog/' + post.slug)}" target="_blank" rel="noopener noreferrer" class="share-btn whatsapp" data-testid="share-whatsapp-top" title="Share on WhatsApp" aria-label="Share on WhatsApp"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg></a>
          <a href="https://twitter.com/intent/tweet?url=${encodeURIComponent('${SITE_URL}/blog/' + post.slug)}&text=${encodeURIComponent(post.title)}" target="_blank" rel="noopener noreferrer" class="share-btn twitter" data-testid="share-twitter-top" title="Share on X/Twitter" aria-label="Share on X/Twitter"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></a>
          <a href="https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent('${SITE_URL}/blog/' + post.slug)}" target="_blank" rel="noopener noreferrer" class="share-btn facebook" data-testid="share-facebook-top" title="Share on Facebook" aria-label="Share on Facebook"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></a>
          <a href="https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent('${SITE_URL}/blog/' + post.slug)}" target="_blank" rel="noopener noreferrer" class="share-btn linkedin" data-testid="share-linkedin-top" title="Share on LinkedIn" aria-label="Share on LinkedIn"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg></a>
          <a href="mailto:?subject=${encodeURIComponent(post.title)}&body=${encodeURIComponent('Check out this article: ${SITE_URL}/blog/' + post.slug)}" class="share-btn email" data-testid="share-email-top" title="Share via Email" aria-label="Share via Email"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 7l-10 7L2 7"/></svg></a>
          <button class="share-btn copy-link" data-testid="share-copy-top" title="Copy Link" aria-label="Copy article link" onclick="copyArticleLink(this)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg><span class="copy-tooltip" role="status" aria-live="polite">Copied!</span></button>
          <span class="share-divider"></span>
          <button class="pdf-btn" data-testid="download-pdf-top" onclick="window.print()" title="Download as PDF" aria-label="Download article as PDF"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Download PDF</button>
        </div>
      </header>
      ...
      <div class="share-bar" data-testid="share-bar-bottom" style="margin-top:1.25rem;margin-bottom:0.5rem">
        <span class="share-label">Share this article</span>
        <a href="https://api.whatsapp.com/send?text=${encodeURIComponent(post.title + ' — Read on Learnpro AI: ${SITE_URL}/blog/' + post.slug)}" target="_blank" rel="noopener noreferrer" class="share-btn whatsapp" data-testid="share-whatsapp-bottom" title="Share on WhatsApp" aria-label="Share on WhatsApp"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg></a>
        <a href="https://twitter.com/intent/tweet?url=${encodeURIComponent('${SITE_URL}/blog/' + post.slug)}&text=${encodeURIComponent(post.title)}" target="_blank" rel="noopener noreferrer" class="share-btn twitter" data-testid="share-twitter-bottom" title="Share on X/Twitter" aria-label="Share on X/Twitter"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></a>
        <a href="https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent('${SITE_URL}/blog/' + post.slug)}" target="_blank" rel="noopener noreferrer" class="share-btn facebook" data-testid="share-facebook-bottom" title="Share on Facebook" aria-label="Share on Facebook"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></a>
        <a href="https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent('${SITE_URL}/blog/' + post.slug)}" target="_blank" rel="noopener noreferrer" class="share-btn linkedin" data-testid="share-linkedin-bottom" title="Share on LinkedIn" aria-label="Share on LinkedIn"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg></a>
        <a href="mailto:?subject=${encodeURIComponent(post.title)}&body=${encodeURIComponent('Check out this article: ${SITE_URL}/blog/' + post.slug)}" class="share-btn email" data-testid="share-email-bottom" title="Share via Email" aria-label="Share via Email"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 7l-10 7L2 7"/></svg></a>
        <button class="share-btn copy-link" data-testid="share-copy-bottom" title="Copy Link" aria-label="Copy article link" onclick="copyArticleLink(this)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg><span class="copy-tooltip" role="status" aria-live="polite">Copied!</span></button>
        <span class="share-divider"></span>
  ...
  <div class="ask-ai-dialog-overlay" id="askAiOverlay" data-testid="ask-ai-dialog-overlay">
    <div class="ask-ai-dialog" role="dialog" aria-labelledby="askAiDialogTitle" data-testid="ask-ai-dialog">
      <div class="ask-ai-dialog-header">
        ...
        <p class="ask-ai-dialog-sub">${post.title.replace(/"/g, '&quot;')}</p>
      </div>
  ...
  function copyArticleLink(btn){
    ...
    if(navigator.clipboard&&navigator.clipboard.writeText){
      navigator.clipboard.writeText(url).then(function(){showCopyTooltip(btn);}).catch(function(){fallbackCopy(url,btn);});
    } else {fallbackCopy(url,btn);}
  }
  function fallbackCopy(text,btn){
    var ta=document.createElement('textarea');ta.value=text;ta.style.position='fixed';ta.style.opacity='0';
    document.body.appendChild(ta);ta.select();
    try{document.execCommand('copy');showCopyTooltip(btn);}catch(e){}
  ...
  function submitAskAi(){
    ...
    document.getElementById('askAiResponseBody').innerHTML='<div class="ask-ai-loading"><div class="ask-ai-loading-dots"><span></span><span></span><span></span></div><span>Generating response\u2026</span></div>';
  ...
  (function(){
    ...
    allPs.forEach(function(p){
      ...
      if(/upsc tip|upsc note|exam tip|important note/i.test(strongTxt)){
        ...
        boxLabel='KEY POINTS';
        iconSvg='<svg viewBox="0 0 24 24" fill="none" stroke="hsl(25,85%,45%)" stroke-width="2"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>';
