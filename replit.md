...
- **Core Modules**:
    ...
    - **PYQ Practice System**: Complete Previous Year Question bank with PDF ingestion pipeline. Features: Admin uploads UPSC/JPSC PDFs → text extraction (pdf-parse + Gemini OCR fallback for scanned PDFs, English-only from bilingual papers) → chunk-based AI structuring (10-15 Qs per chunk) → validation → **Language Filter Agent** (removes any remaining Hindi/Devanagari questions) → **Structure Fix Agent** (AI-powered batch formatting cleanup: merges broken multi-line options/sentences, fixes match-the-column formatting, extracts options from questionText when placeholder options exist) → taxonomy-constrained topic classification + difficulty → textHash deduplication → database insert. Admin cleanup: `POST /admin/api/pyq/fix-structure` re-runs structure fix on broken questions only (async, runs in background). **Frontend rendering**: `mergeQuestionLines()` function intelligently joins broken PDF lines before rendering (merges continuation lines, preserves structural breaks for numbered lists and option letters). `FormattedQuestionText` strips embedded (a)-(d) option lines from question text when separate options array exists. `examStage` field ("Prelims"/"Mains") separates stages throughout. Prelims = instant MCQ scoring; Mains = AI evaluation with structured rubric (Introduction/Body/Conclusion/ContentCoverage + strengths/improvements). Topic trends with weighted predictions (freq×0.6 + recency×0.3 + streak×0.1). "Asked Before" tags integrated in Chat, Current Affairs, and Study Planner. Controlled taxonomy: 13 topics (no "Unclassified" — all questions must be classified into a real topic), 80+ subtopics. If AI classification fails during ingestion, a single-question retry is attempted; only as a last resort does it default to the first topic in the list. Admin CRUD, bulk JSON import. **Background Job Queue**: PDF ingestion runs in background via `pyq_ingestion_jobs` table + `server/pyq-worker.ts`. Upload saves PDF, creates job (status: queued), worker polls every 3s and processes sequentially. Atomic job claiming via `FOR UPDATE SKIP LOCKED`. Stale job recovery (re-queues processing jobs older than 5 min on restart). Admin UI shows real-time progress bars with auto-polling. **Job Cancel/Stop**: Processing or queued jobs can be stopped via `POST /admin/api/pyq/jobs/:id/cancel`. Uses in-memory `cancelledJobs` Set + `isCancelled` callback checked at every processing phase (chunk extraction, classification, DB insert). Admin UI shows Stop button for queued/processing, Retry for failed/cancelled. "Clear Finished" button removes all completed/failed/cancelled jobs. APIs: `POST /admin/api/pyq/queue`, `GET /admin/api/pyq/jobs`, `DELETE /admin/api/pyq/jobs/:id`, `POST /admin/api/pyq/jobs/:id/retry`, `POST /admin/api/pyq/jobs/:id/cancel`, `POST /admin/api/pyq/jobs/clear-finished`. Tables: `pyq_questions`, `pyq_attempts`, `pyq_ingestion_jobs`. Routes: `server/pyq-routes.ts`, `server/pyq-worker.ts`. Frontend: `client/src/pages/pyq-page.tsx` at `/pyq`. Admin section in admin panel under "PYQ Bank".
