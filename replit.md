...
- **Core Modules**:
    ...
    - **AI Content Quality**: Prompts enforce: single search intent per article, data tables with real numbers, year-specific references, trend analysis, internal linking to existing articles, anti-hallucination rules, 50+ banned AI-inflated phrases. Titles must include numbers/specific claims. English only. Voice: direct mentor, not coaching center blog.
    - **Blog Design**: Warm amber/gold + deep blue hero. Plus Jakarta Sans for headings, Inter for body. Listing page: category tabs with arrows, right sidebar (date/month archives, tags, CTA). Article detail: amber tables with borders, FAQ accordion, ToC sidebar, related articles. All server-rendered.
