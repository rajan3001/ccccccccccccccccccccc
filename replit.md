...
- 2026-02-08: Improved Current Affairs - now sources from The Hindu, Indian Express, and state-specific newspapers (15 state newspapers mapped). Each topic shows newspaper source name. Updated prompts to pick real news articles, not random content.
- 2026-02-08: Redesigned Current Affairs to SuperKalam-style layout - two-column with topics grouped by newspaper source, colored GS badges, clickable topic cards with right chevron arrows. Clicking a topic opens a dedicated detail page (/current-affairs/topic/:id) with breadcrumb navigation, GS badges, headline, summary, Download/Share buttons, "Attempt MCQs" card, and streaming AI detailed analysis. Right sidebar shows MCQ CTA card and Daily Progress calendar.
- 2026-02-08: Switched Current Affairs sourcing from The Hindu scraper to NextIAS (nextias.com/ca/current-affairs/) scraper. NextIAS already curates UPSC-relevant topics with GS categories and source newspaper references. Topics are stored directly from scraping (no AI needed for generation). Falls back to AI-only generation if NextIAS scraping fails. Page numbers estimated from topic category. Empty digests are auto-cleaned for regeneration.
